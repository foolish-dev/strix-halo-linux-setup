...
#### Repository Implementation
```bash
# Kernel command-line parameters (manual opt-in — no script in this repository
# applies them). The toolkit writes cmdline parameters through whichever of
# GRUB, systemd-boot, rEFInd or Limine it detects; see
# distro_configure_amd_pstate in strix-halo-lib/distro-manager.sh.
amdgpu.gttsize=131072  # 128 GiB Graphics Translation Table (gttsize is in MiB)
...
- LLM/AI workloads: apply the GTT optimization **manually** — `modules/llm.sh`
  does not set `gttsize`, and nothing in the repository does
...
**Analysis:**
- **Functionality:** Wrappers around `z13ctl` (generated by `z13ctl_generate_wrappers` in `strix-halo-setup.sh`)
- **Value Proposition:** Convenient power profile management specific to GZ302 thermal envelope
- **Dependency:** Requires `z13ctl`, ASUS devices only (gated on `CAP_Z13CTL`)
